...
Feature: Get data from the media library in json format
  ...
  Scenario: Get all files from a media lib package
    Given I have a request header "HTTP_ACCEPT" with value "application/json"
    And I request "GET" "/api/media/package/looks?&attributes=id,name,flavors,category,author,extension"
  ...
  Scenario: Get all files from a media lib package with limit = 1
    Given I have a request header "HTTP_ACCEPT" with value "application/json"
    And I request "GET" "/api/media/package/looks?limit=1&attributes=id,name,flavors,packages,category,author,extension,download_url"
    Then the response status code should be "200"
    And I should get the json object:
    """
    [
  ...
  Scenario: Get all files from a media lib package with limit = 1 and offset = 3
    Given I have a request header "HTTP_ACCEPT" with value "application/json"
    And I request "GET" "/api/media/package/looks?limit=1&offset=3&attributes=id,name,flavors,packages,category,author,extension,download_url"
    Then the response status code should be "200"
    And I should get the json object:
    """
    [
